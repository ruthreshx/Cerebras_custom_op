...
@pytest.mark.parametrize(
    "input, batch1, batch2",
    (
        (torch.randn(3, 2), torch.randn(4, 3, 3), torch.randn(4, 3, 2)),  # Regular input and matrices
        (torch.zeros(2, 3), torch.ones(4, 2, 3), torch.ones(4, 3, 3)),  # Zeros input, ones for mat1, mat2
        
        # Varying tensor shapes
        (torch.randn(3, 2), torch.randn(6, 3, 3), torch.randn(6, 3, 2)),  # Larger batch size for mat1 and mat2
        (torch.randn(5, 2), torch.randn(4, 5, 5), torch.randn(4, 5, 2)),  # Higher dimension tensors for batch
    ),
)
